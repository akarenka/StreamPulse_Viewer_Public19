...
live.html
firebase-config.js
firestore.rules (publish this in Firebase Console; do not serve it as app configuration)
```
...
profiles/{uid}
userMediaSettings/{uid}/personalMeta/{mediaId}
userMediaSettings/{uid}/creatorGroups/{groupId}
userMediaSettings/{uid}/ui/creatorGrouping
```

The main media lobby uses real Firebase Google Authentication. It synchronizes
the profile nickname/avatar, Creator Name edits, collapsible group covers, and
grouping mode privately under the signed-in UID. Existing local Creator metadata
and covers are migrated once for that Google account after the first successful login.
